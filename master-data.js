        ...
        <div class="broker-account-item" id="broker-account-${index}">
            ...
            <div class="brokerage-grid">
                ...
                <div class="brokerage-section">
                    ...
                    <div class="form-row">
                        <div class="form-group"><label>%</label><input type="number" step="0.01" class="eq-del-pct" data-index="${index}" value="${rates.equity?.delivery?.pct !== undefined ? rates.equity.delivery.pct : 0}"></div>
                        <div class="form-group"><label>Max ₹</label><input type="number" step="0.01" class="eq-del-max" data-index="${index}" value="${rates.equity?.delivery?.max !== undefined ? rates.equity.delivery.max : 20}"></div>
                    </div>
                </div>
                <div class="brokerage-section">
                    <span class="brokerage-label">Equity - Intraday</span>
                    <div class="form-row">
                        <div class="form-group"><label>%</label><input type="number" step="0.01" class="eq-intra-pct" data-index="${index}" value="${rates.equity?.intraday?.pct !== undefined ? rates.equity.intraday.pct : 0.03}"></div>
                        <div class="form-group"><label>Max ₹</label><input type="number" step="0.01" class="eq-intra-max" data-index="${index}" value="${rates.equity?.intraday?.max !== undefined ? rates.equity.intraday.max : 20}"></div>
                    </div>
                </div>
                <div class="brokerage-section">
                    <span class="brokerage-label">Futures</span>
                    <div class="form-row">
                        <div class="form-group"><label>%</label><input type="number" step="0.01" class="fut-pct" data-index="${index}" value="${rates.derivatives?.futures?.pct !== undefined ? rates.derivatives.futures.pct : 0.03}"></div>
                        <div class="form-group"><label>Max ₹</label><input type="number" step="0.01" class="fut-max" data-index="${index}" value="${rates.derivatives?.futures?.max !== undefined ? rates.derivatives.futures.max : 20}"></div>
                    </div>
                </div>
                <div class="brokerage-section">
                    <span class="brokerage-label">Options</span>
                    <div class="form-row">
                        <div class="form-group"><label>Flat ₹</label><input type="number" step="0.01" class="opt-flat" data-index="${index}" value="${rates.derivatives?.options?.flat !== undefined ? rates.derivatives.options.flat : 20}"></div>
                        <div class="form-group"><label>Max ₹</label><input type="number" step="0.01" class="opt-max" data-index="${index}" value="${rates.derivatives?.options?.max !== undefined ? rates.derivatives.options.max : 0}"></div>
                    </div>
                </div>
            </div>
        </div>
...
async function loadBrokerDefaults(index) {
    ...
    document.querySelector(`.eq-del-pct[data-index="${index}"]`).value = rates.equity?.delivery?.pct ?? 0;
    document.querySelector(`.eq-del-max[data-index="${index}"]`).value = rates.equity?.delivery?.max ?? 20;
    document.querySelector(`.eq-intra-pct[data-index="${index}"]`).value = rates.equity?.intraday?.pct ?? 0.03;
    document.querySelector(`.eq-intra-max[data-index="${index}"]`).value = rates.equity?.intraday?.max ?? 20;
    document.querySelector(`.fut-pct[data-index="${index}"]`).value = rates.derivatives?.futures?.pct ?? 0.03;
    document.querySelector(`.fut-max[data-index="${index}"]`).value = rates.derivatives?.futures?.max ?? 20;
    document.querySelector(`.opt-flat[data-index="${index}"]`).value = rates.derivatives?.options?.flat ?? 20;
    document.querySelector(`.opt-max[data-index="${index}"]`).value = rates.derivatives?.options?.max ?? 0;
...
function openAddBrokerModal() {
    ...
    document.getElementById('eqDelPct').value = 0;
    document.getElementById('eqDelMax').value = 20;
    document.getElementById('eqIntraPct').value = 0.03;
    document.getElementById('eqIntraMax').value = 20;
    document.getElementById('futPct').value = 0.03;
    document.getElementById('futMax').value = 20;
    document.getElementById('optFlat').value = 20;
    document.getElementById('optMax').value = 0;
...
async function editBroker(id) {
    ...
    const rates = broker.default_brokerage_rates || {};
    document.getElementById('eqDelPct').value = rates.equity?.delivery?.pct ?? 0;
    document.getElementById('eqDelMax').value = rates.equity?.delivery?.max ?? 20;
    document.getElementById('eqIntraPct').value = rates.equity?.intraday?.pct ?? 0.03;
    document.getElementById('eqIntraMax').value = rates.equity?.intraday?.max ?? 20;
    document.getElementById('futPct').value = rates.derivatives?.futures?.pct ?? 0.03;
    document.getElementById('futMax').value = rates.derivatives?.futures?.max ?? 20;
    document.getElementById('optFlat').value = rates.derivatives?.options?.flat ?? 20;
    document.getElementById('optMax').value = rates.derivatives?.options?.max ?? 0;
